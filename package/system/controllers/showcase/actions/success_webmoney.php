...
class actionShowcaseSuccessWebmoney extends cmsAction {
    ...
    public function run(){
		...
		if ($order_id){
			...
			if ($transaction){
				$transaction = $this->scUpdateTransaction($transaction, 'history', 'Получен ответ успешного платежа');
			} else {
				$tData = array(
					'order_id' => $order_id ? $order_id : 0,
					'system_id' => 0,
					'history' => array('Получен ответ успешного платежа'),
				);
