...
class Query
{
    ...
    protected static function parseProperty($property)
    {
        ...
        if (preg_match($regexp, $property, $segments)) {
            ...
            $result['args'] = isset($segments['args']) ? explode('|', $segments['args']) : array();
     ...
     */
    protected static function convertProperty($name, $args = array())
    {
        if ($name === 'text') {
            return 'text()';
        }
    ...
    {
        $isSimpleSelector = !in_array(substr($name, 0, 1), array('^', '!'));
        $isSimpleSelector = $isSimpleSelector && (!in_array(substr($name, -1), array('^', '$', '*', '!', '~')));
     ...
     */
    protected static function convertPseudo($pseudo, $parameters = array(), &$tagName)
    {
        switch ($pseudo) {
            case 'first-child':
                return 'position() = 1';
