...
class Document
{
     ...
     */
    public function createElementBySelector($selector, $value = null, $attributes = array())
    {
    ...
    public function load($string, $isFile = false, $type = 'html', $options = 0)
    {
        ...
        if (!in_array(strtolower($type), array('xml', 'html'))) {
        ...
        $this->type === 'xml' ? $this->document->loadXml($string, $options) : $this->document->loadHtml($string);
